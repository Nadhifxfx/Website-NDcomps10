          ...
          <div className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-white font-bold text-sm tracking-wider hover:text-[#FFD700] transition-colors relative group"
              >
                {link.label}
          ...
          <div className="px-4 py-4 space-y-3">
            {navLinks.map((link) => (
              <Link
                ...
                className="block text-white font-bold text-lg tracking-wider hover:text-[#FFD700] transition-colors py-2"
              >
                {link.label}
              </Link>
            ))}
